    ...
    def get_query(self):
        if self.lemma:
            ...
            query = 'SELECT DISTINCT c.name, p.value, p.description FROM lemma l ' \
                    ...
                    f'JOIN category c on p.category_id = c.id WHERE ({usage_types_query_part}) AND l.value = ?'
            return query, [self.lemma]
            ...
            query = f' SELECT DISTINCT c.name, p.value, p.description FROM proverb p ' \
                    ...
                    f'WHERE substr(p.value,1,1) IN(?, ?)'
            return query, [self.first_proverb_letter.lower(), self.first_proverb_letter.upper()]
        elif self.substring:
            query = f'SELECT DISTINCT c.name, p.value, p.description FROM proverb p ' \
                    f'JOIN category c ON p.category_id = c.id ' \
                    f'WHERE p.value LIKE (\'%\' || ? || \'%\') OR ' \
                    f'p.description LIKE (\'%\' || ? || \'%\')'
            return query, [self.substring, self.substring]


def search_proverbs(filter, db):
    query, params = filter.get_query()
...
def search_lemmas_by_first_letter(letter, usage_type, db):
    rows = db.select_all('SELECT DISTINCT (l.value) FROM lemmas_usage u JOIN lemma l ON u.lemma_id = l.id '
                         'WHERE u.usage_type = ? AND substr(l.value,1,1) = ? ORDER BY l.value', usage_type, letter.lower())
